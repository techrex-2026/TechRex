...
@app.route('/')
def portal():
    # En PythonAnywhere no hay forma de saber la IP local de la
    # computadora que en ese momento tiene el hardware conectado, asi
    # que se fija a mano. ACTUALIZAR ESTE VALOR EL DIA DEL EVENTO,
    # con la IP local real de esa red (comando "ipconfig" en Windows).
    IP_LOCAL_COMPETENCIA = "PENDIENTE"
    return render_template_string(PAGINA_PORTAL,
                                  url_escaner=f"http://{IP_LOCAL_COMPETENCIA}:{PUERTO_ESCANER}/")
...
# Clave compartida con web/empujar_datos.py. PythonAnywhere no puede leer
# el puerto serial directamente, asi que este endpoint recibe los datos
# que el script local ya leyo del Arduino y los reenvia por internet.
CLAVE_PUSH = "rexscan2026"

@app.route('/api/recibir_datos', methods=['POST'])
def recibir_datos():
    global ultimo_dato, historial
    datos = request.get_json(silent=True) or {}
    if datos.get("clave") != CLAVE_PUSH:
        return jsonify({"error": "no autorizado"}), 403
    ultimo_dato = datos.get("actual", ultimo_dato)
    historial = datos.get("historial", historial)
...
# NOTA PYTHONANYWHERE: esta version NO lee el puerto serial porque el
# servidor no tiene acceso fisico al Arduino. Los datos llegan por
# HTTP a traves de /api/recibir_datos, empujados por web/empujar_datos.py
# desde la computadora que si tiene el hardware conectado.
#
...
#
# PythonAnywhere ejecuta la app a traves de su propio sistema WSGI,
# no con este bloque — se deja comentado solo como referencia.
